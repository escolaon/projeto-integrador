          ...
          <form @submit.prevent="handleSave">
            <div class="grid w-full items-center gap-4">
              <div class="flex flex-col space-y-1.5">
                ...
                <select id="turma" v-model="selectedTurmaId" @change="updateSelectedTurmaName" class="alert-input">
                  <option v-for="turma in turmas" :key="turma.id" :value="turma.id">{{ turma.nome }}</option>
                </select>

                <UiLabel for="aluno">Aluno</UiLabel>
                <select id="aluno" v-model="selectedAlunoId" @change="updateSelectedAlunoName" class="alert-input">
                  <option v-for="aluno in alunos" :key="aluno.id" :value="aluno.id">{{ aluno.nome }}</option>
                </select>

                <UiLabel for="professor">Professor</UiLabel>
                <select id="professor" v-model="selectedProfessorId" @change="updateSelectedProfessorName" class="alert-input">
                ...
                <select id="disciplina" v-model="selectedDisciplinaId" @change="updateSelectedDisciplinaName" class="alert-input">
...
const newOcorrencia = reactive({
  ...
  async function handleSave() {
      const response = await $fetch(`${config.public.url}/api/ocorrencia`, {
        ...
        body: JSON.stringify({ turmaId: selectedTurmaId.value, turmaNome: selectedTurmaNome.value, alunoId: selectedAlunoId.value, alunoNome: selectedAlunoNome.value, professorId: selectedProfessorId.value, professorNome: selectedProfessorNome.value, disciplinaId: selectedDisciplinaId.value, disciplinaNome: selectedDisciplinaNome.value }),
    });
