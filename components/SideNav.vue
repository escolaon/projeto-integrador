    ...
    <template v-for="(l, i) in links" :key="i">
      <div v-if="!l.children" class="flex items-center gap-4">
        <NuxtLink
          ...
          exact-active-class="underline underline-offset-2" :to="l._path" :title="l.title">
          {{ l.title }}
        </NuxtLink>
      ...
      <div v-else class="flex flex-col gap-2">
        <div class="flex items-center gap-2.5">
          <Icon v-if="l.icon" :name="l.icon" class="h-6 w-6" />
          <p class="text-[17px] font-medium sm:text-sm">{{ l.title }}</p>
        </div>
        <div class="pl-8">
          <SideNav class="gap-2" :links="l.children" />
        </div>
      </div>
    </template>
  </nav>
